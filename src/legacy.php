...
if (! function_exists('cleancoded_star_ratings')) {
    function cleancoded_star_ratings($idOrPost = null)
    {
        $id = null;
        ...
        return cleancoded\StarRating\response(compact('id'), false);
    }
}

if (! function_exists('cleancoded_star_ratings_get')) {
    function cleancoded_star_ratings_get($limit = 5, $taxonomyId = null, $offset = 0)
    {
        global $wpdb;
        $postsTable = $wpdb->posts;
        $postMetaTable = $wpdb->prefix.'postmeta';
        $base = get_option(cleancoded\StarRating\prefix('stars')) / 5;
        ...
        $queryConditions = "
            WHERE
                ...
                AND postmeta_count.meta_key = '_cleancoded_casts'
                AND postmeta_ratings.meta_key = '_cleancoded_ratings'
        ";
