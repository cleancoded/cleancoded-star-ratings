...
function ajax()
{
    if (! check_ajax_referer(config('slug').'-ajax', 'nonce', false)) {
        ...
        return wp_die(json_encode([
            'error' => __('This action is forbidden.', 'cleancoded-star-ratings'),
        ]));
    }
    ...
    if (! isset($_POST['id'])) {
        ...
        return wp_die(json_encode([
            'error' => __('An id is required to vote.', 'cleancoded-star-ratings'),
        ]));
    }
    ...
    if (! apply_plugin_filters('can_vote', true, $id, $slug)) {
        ...
        return wp_die(json_encode([
            'error' => __('You are not allowed to vote.', 'cleancoded-star-ratings'),
        ]));
    }
    ...
    if (! isset($_POST['score'])) {
        ...
        return wp_die(json_encode([
            'error' => __('A rating is required to vote.', 'cleancoded-star-ratings'),
        ]));
    }
